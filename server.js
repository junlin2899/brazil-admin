...
// Regenerate products.js for frontend
app.post('/api/regenerate', (req, res) => {
  try {
    const products = readProducts();
    const productsJsContent = `// Auto-generated by admin backend
...
`;
    const productsJsPath = path.join(PUBLIC_DIR, 'js', 'products.js');
    fs.writeFileSync(productsJsPath, productsJsContent, 'utf-8');
    res.json({ success: true, count: products.length, message: '已同步 ' + products.length + ' 个商品到前台' });
  } catch (err) {
